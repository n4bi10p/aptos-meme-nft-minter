        ...
        <div className="absolute right-0 mt-2 w-64 card animate-slide-up">
          {connected ? (
            <button
              ...
              className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-xl transition-colors text-red-600 font-medium"
            >
              Disconnect Wallet
            </button>
          ) : (
            <div className="space-y-2">
              <p className="text-sm text-gray-500 mb-3">Choose your wallet:</p>
              {wallets.map((availableWallet) => (
                ...
                >
                  ...
                  />
                  <span className="font-medium">{availableWallet.name}</span>
                </button>
              ))}
            </div>
          )}
